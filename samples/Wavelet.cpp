...
double** SeparateRows(double** data, int rows, int cols)
{
	...
	for (int i = 0; i < rows; i++) {
		move = 0;
		for (int j = 0; j < cols; j += 2) {
			tmp_data[i][move] = data[i][j];
			tmp_data[i][cols / 2 + move] = data[i][j + 1];
			move++;
		}
	}
	return tmp_data;
}
double** SeparateCols(double** data, int rows, int cols)
{
	double** tmp_data = new double*[rows];
	for (int i = 0; i < rows; i++)
		tmp_data[i] = new double[cols];
	...
	for (int j = 0; j < cols; j++) {
		move = 0;
		for (int i = 0; i < rows;  i+= 2) {
			tmp_data[move][j] = data[i][j];
			tmp_data[rows/2+move][j] = data[i+1][j];
			move++;
		}
	}
	return tmp_data;
...
int main() 
{
	...
	data = SeparateRows(data, image.rows, image.cols);
	WaveletCols(data, image.rows, image.cols);
	data = SeparateCols(data, image.rows, image.cols);
	//PrintData(data, image.rows, image.cols);
	imshow("image", image);
	imshow("wl1", DataToMat(data, image.rows, image.cols));
	waitKey();
	for (int i = 0; i < image.rows; i++)
		delete[] data[i];
	delete[] data;
